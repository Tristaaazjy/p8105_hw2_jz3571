...
```{r}
merge_df = merge(tidy_pol, tidy_snp) %>% 
  merge(tidy_unemp)
```

The combined data set includes `r nrow(merge_df)` rows and `r ncol(merge_df)` columns. The year range be included in the data is `r max(merge_df$year)-min(merge_df$year)` years. Some other key variables include: year, month, the number of republican and democratic governors, senators, and representatives respectively on the associated date, the party of the president on the associated date, the closing value and the unemployment rate on the associated date.
